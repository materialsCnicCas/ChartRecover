...
class CocoMetric(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        # 计算额外IoU的AP，例如0.5, 0.7, 0.9
        additional_iou_thrs = [0.5, 0.7, 0.9]
        for iou in additional_iou_thrs:
            # 寻找对应的索引
            iou_idx = np.where(np.isclose(self.iou_thrs, iou))[0]
            if len(iou_idx) == 0:
                continue
            iou_idx = iou_idx[0]
            # 计算该iou下的AP，所有类别、所有面积、maxDets=100
            ap_iou = coco_eval.eval['precision'][iou_idx, :, :, 0, -1].mean()
            key = f'{metric}_mAP_{int(iou*100)}'
            eval_results[key] = round(ap_iou, 4)
            logger.info(f'{key}: {ap_iou:.4f}')
